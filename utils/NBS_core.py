...
def network_inf_KNN_glap(network, gamma=0.01, kn=11, verbose=True, **save_args):
    ...
    if verbose:
        print('Graph laplacian of KNN network from influence matrix calculated:', time.time() - KNN_starttime,
              'seconds')
    return knnGlap
...
# patients with fewer than 10 mutations were discarded
def subsample_sm_mat(sm_mat, propNet=None, pats_subsample_p=0.8, gene_subsample_p=0.8, min_muts=10):
    ...
    # Sub sample patients
    pats_subsample = random.sample(set(sm_mat.index), int(Nsample))
    # Sub sample genes
    gene_subsample = random.sample(set(sm_mat.columns), int(Dsample))
    # Sub sampled data mat
    gind_sample = sm_mat.loc[pats_subsample][gene_subsample]
    # Filter by mutation count
    gind_sample = gind_sample[gind_sample.sum(axis=1) > min_muts-1]
    # Filter columns by network nodes only if network is given
    # gind_sample_filt = gind_sample
    gind_sample_filt = pd.DataFrame()
    if propNet is not None:
        ...
        for key in propNet.nodes:
            if key in gind_sample.columns:
                gind_sample_filt.insert(len(gind_sample_filt.columns), key, gind_sample[key])
            else:
                gind_sample_filt.insert(len(gind_sample_filt.columns), key, 0)
...
def mixed_netNMF(data, KNN_glap, k=3, l=200, maxiter=250,
    ...
    # Initialize W
    W_init = np.linalg.lstsq(H.T, data.T, rcond=-1)[0].T
