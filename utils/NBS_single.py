...
def NBS_single(sm_mat, regNet_glap, propNet=None, propNet_kernel=None,
    ...
    # pats_subsample_p, gene_subsample_p, min_muts = 1.0, 1.0, 2
    ...
    sm_mat_subsample = core.subsample_sm_mat(sm_mat, propNet=propNet,
                                             pats_subsample_p=pats_subsample_p, gene_subsample_p=gene_subsample_p,
                                             min_muts=min_muts)
    if verbose:
    ...
    if propNet is not None:
        ...
        if propNet_kernel is None:
            ...
            if save_prop:
                prop_sm_data = prop.network_propagation(propNet, sm_mat_subsample, alpha=alpha,
                                                        symmetric_norm=symmetric_norm, **kwargs)
            else:
                prop_sm_data = prop.network_propagation(propNet, sm_mat_subsample, alpha=alpha,
    ...
    W, H, numIter, finalResid = core.mixed_netNMF(data_arr, regNet_glap_arr, k=k,
                                                  ...
                                                  err_tol=netNMF_err_tol, err_delta_tol=netNMF_err_delta_tol,
                                                  verbose=False)
