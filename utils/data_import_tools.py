...
# Loads any non-default optional parameters that may be used in the NBS pipeline
...
def load_params(params_file=None):
    ...
    if params_file is not None:
        ...
        for param in params_file.index:
            if param in run_NBS_params:
                if type(run_NBS_params[param]) == bool:
                    run_NBS_params[param] = (params_file.loc[param, 'value'] == 'True')
                else:
                    run_NBS_params[param] = params_file.loc[param].astype(type(run_NBS_params[param]))['value']
            else:
                run_NBS_params[param] = params_file['value'].loc[param]
    else:
        pass
    # Constructs output directory if directory does not exist
    if not os.path.exists(run_NBS_params['outdir']):
        os.makedirs(run_NBS_params['outdir'])
    return run_NBS_params
    ...
    try:
        ...
        if verbose:
            print('Note: Maximum number of swap attempts (' + repr(
                edge_len * 10) + ') exceeded before desired swaps achieved (' + repr(edge_len) + ').')
    if verbose:
...
def filter_weighted_network(network_file_path, save_path, nodeA_col=0, nodeB_col=1, score_col=2, q=0.9, delimiter='\t',
                            verbose=False):
